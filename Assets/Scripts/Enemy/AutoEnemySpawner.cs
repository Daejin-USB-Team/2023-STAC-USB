...
public class AutoEnemySpawner : MonoBehaviour
{
	...
	private AutoWave currentWave;           // 현재 웨이브 정보
	private int currentEnemyCount;      // 현재 웨이브에 남아있는 적 숫자 (웨이브 시작시 max로 설정, 적 사망 시 -1)
	private List<AutoEnemy> enemyList;              // 현재 맵에 존재하는 모든 적의 정보
	...
	private IEnumerator SpawnEnemy()
	{
		...
		while (spawnEnemyCount < currentWave.maxEnemyCount)
		{
			...
			AutoEnemy enemy = clone.GetComponent<AutoEnemy>();  // 방금 생성된 적의 Enemy 컴포넌트
